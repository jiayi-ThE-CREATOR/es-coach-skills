---
name: kigyou-report
description: 就活向けの企業分析レポートを蒸馏システム風（構造化）で生成する。Vault 既存ノート + Web リサーチを組み合わせ、Properties / Callout / テーブル / Mermaid を駆使した 12 セクションの深度レポートを「就活/企業分析报告/」に保存する。/kigyou-report <企業名> で起動。外資系金融・総合商社・戦略コンサル・日系大手など、どの業界の企業でも対応可能。
---

# kigyou-report — 就活企業分析レポート生成 skill

ユーザーが指定した企業を 12 セクション固定の構造化レポートに蒸馏し、Obsidian Vault の「企業分析报告/」に保存する skill（公開情報が薄い企業のみ簡易 5 版に縮退。エッジケース参照）。
...
VAULT_SHUKATSU_DISTILL   → 就活蒸馏システムの根目録（素材参照元は 素材/ 配下、蒸馏知識参照元は 蒸馏知識/ 配下）
...
3. **蒸馏知識**（`${VAULT_SHUKATSU_DISTILL}/蒸馏知識/内定之路/<業界>.md`）
...
9. **内定者の共通パターン** — Vault 蒸馏データから 5 つ抽出、学歴傾向、強み事例
...
**Mermaid テンプレ**（このまま使用、企業固有部門名のみ差し替え）：
